...
*/}}
{{- define "kube-prometheus.volumePermissions.image" -}}
...
{{ include "common.images.pullSecrets" (dict "images" (list .Values.operator.image .Values.operator.prometheusConfigReloader.image .Values.volumePermissions.image .Values.prometheus.image .Values.prometheus.thanos.image .Values.alertmanager.image) "global" .Values.global) }}
{{- end -}}
...
{{/*
Return the proper Prometheus ingress host
*/}}
{{- define "prometheus.ingressHost" -}}
{{- if .Values.global }}
    {{- if .Values.global.innerHost }}
     {{- printf "prometheus.%s" .Values.global.innerHost -}}
    {{- end -}}
{{- else }}
{{- printf "%s" .Values.prometheus.ingress.hostname -}}
{{- end -}}
{{- end -}}

{{/*
Return the proper Alertmanager ingress host
*/}}
{{- define "alertmanager.ingressHost" -}}
{{- if .Values.global }}
    {{- if .Values.global.innerHost }}
     {{- printf "alertmanager.%s" .Values.global.innerHost -}}
    {{- end -}}
{{- else }}
{{- printf "%s" .Values.alertmanager.ingress.hostname -}}
{{- end -}}
{{- end -}}

{{/*
Return the proper Alertmanager target name
*/}}
{{- define "alertmanager.targetName" -}}
{{- if .Values.global }}
    {{- if .Values.global.targetName }}
     {{- print .Values.global.targetName -}}
    {{- end -}}
{{- else }}
{{- print "默认环境名" -}}
{{- end -}}
